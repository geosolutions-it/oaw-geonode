...
invoke () {
    ...
    then
        ...
        /usr/local/bin/invoke $@ >> /usr/src/oaw_geonode4/invoke.log 2>&1
    fi
    echo "$@ tasks done"
}
...
then
    ...
    then
        ...
        env -u http_proxy -u https_proxy invoke devrequirements
        env -u http_proxy -u https_proxy invoke statics
    ...
    then
        ...
        if [ ${FORCE_REINIT} = "true" ]  || [ ${FORCE_REINIT} = "True" ] || [ ! -e "/mnt/volumes/statics/geonode_init.lock" ]; then
            echo "LOG INIT" > /usr/src/oaw_geonode4/invoke.log
            invoke updategeoip
            invoke fixtures
        ...
        invoke statics
        env -u http_proxy -u https_proxy invoke waitforgeoserver
        env -u http_proxy -u https_proxy invoke geoserverfixture
